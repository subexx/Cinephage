...
describe('StremioAddonService', () => {
	...
	it('formats stream name and description with the AIOMedia template', async () => {
		...
		expect(streams[0].url).toBe(
			'https://media.example.com/api/streaming/session/movie/603?api_key=stream-key'
		);
		...
		expect(streams[1].url).toBe(
			'https://media.example.com/api/streaming/library/movie/f-mkv?api_key=stream-key'
		);
	});
	...
	it('resolves episode streams from imdb season/episode ids', async () => {
		...
		expect(streams[0].url).toBe(
			'https://media.example.com/api/streaming/session/tv/1399/1/1?api_key=k'
		);
	});
